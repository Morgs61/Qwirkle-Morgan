    ...
    for (int i = 0; i < ROWS; i++) {
        ...
        for (int j = 0; j < ROWS; ++j) {
            ...
            if (board[i * COLS + j] != nullptr) {
                ...
                cout << "|"
                     << tileString;  // Print the tile's string representation
            } else {
...
std::string Board::getBoardState() {
    ...
    for (int i = 0; i < ROWS; i++) {
        for (int j = 0; j < COLS; ++j) {
            if (board[i * COLS + j] != nullptr) {
                stateString += board[i * COLS + j]->toString() + "@" +
                               labels[i] + to_string(j) + ", ";
            }
        }
    }
...
bool Board::checkSurroundingTilesMatch(int row, int col, Tile *tile) {
    ...
    std::vector<std::vector<Tile *>> board2D(
        ROWS, std::vector<Tile *>(COLS, nullptr));
    for (int i = 0; i < ROWS; ++i) {
        for (int j = 0; j < COLS; ++j) {
    ...
    bool isBoardEmpty =
        std::all_of(board2D.begin(), board2D.end(),
                    [](const std::vector<Tile *> &boardRow) {
                        return std::all_of(
                            boardRow.begin(), boardRow.end(),
                            [](const Tile *tile) { return tile == nullptr; });
                    });
    ...
    bool rowMatch = true;
    for (int i = col - 1; i >= 0 && board2D[row][i] != nullptr && rowMatch;
         --i) {
    ...
    bool colMatch = true;
    for (int i = row - 1; i >= 0 && board2D[i][col] != nullptr && colMatch;
         --i) {
...
int Board::calculateScore(const std::vector<Tile *> &tilesToPlace,
    ...
    for (size_t i = 0; i < tilesToPlace.size(); ++i) {
        ...
        if (scoredPositions.count({row, col}) == 0) {
            ...
            // Check left
            for (int j = col - 1; j >= 0 && board[row * COLS + j] != nullptr;
                 --j) {
                scoreHorizontal++;
                scoredPositions.insert(
                    {row, j});  // Mark this position as scored
            }
            // Check right
            for (int j = col + 1; j < COLS && board[row * COLS + j] != nullptr;
                 ++j) {
                scoreHorizontal++;
                scoredPositions.insert(
                    {row, j});  // Mark this position as scored
            ...
            // Check up
            for (int j = row - 1; j >= 0 && board[j * COLS + col] != nullptr;
                 --j) {
                scoreVertical++;
                scoredPositions.insert(
                    {j, col});  // Mark this position as scored
            }
            // Check down
            for (int j = row + 1; j < ROWS && board[j * COLS + col] != nullptr;
                 ++j) {
                scoreVertical++;
                scoredPositions.insert(
                    {j, col});  // Mark this position as scored
