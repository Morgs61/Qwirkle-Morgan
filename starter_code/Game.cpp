    ...
    while (!playerTurnComplete) {
      ...
      if (menuChoice == 1) {  // Place tiles
        ...
        if (currentPlayer->getHand()->getSize() < 6) {
          cout << "You have already placed a tile. You can not now replace a "
                  "tile"
               << endl;
          // return;
        } else {
...
bool Game::validateTilePlacement(vector<string>& words,
  ...
  if (!currentPlayer->getHand()->containsTile(tileToCheck)) {
    ...
    cout << "Invalid move. Tiles must have the same color, shape, and share "
...
void Game::updateGameAfterTilePlacement(
    vector<Tile*>& tilesToPlace, vector<std::pair<int, int>>& tilePositions,
    int& numTiles) {
...
bool Game::placeTiles() {
  ...
  while (!activeTurn && currentPlayer->getHand()->getSize() > 0) {
    currentPlayer->getHand()->displayHand();
    cout << "Place tile " << numTiles + 1
         << " using the format: place <tile> at <grid location>" << endl;
    cout << "Enter 'end' to end your turn or 'back' to return to previous menu"
         << endl;
    cout << "> ";
    ...
    if (words.size() == 1 && words[0] == "back") {
      if (numTiles > 0) {
        cout << "You have already placed a tile. You must continue your move."
             << endl;
        // activeTurn = false;
      } else {
